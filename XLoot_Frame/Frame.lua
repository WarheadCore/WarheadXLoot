...
function XLootFrame:ParseAutolootList()
	...
	for item in opt.autoloot_blackitem_list:gmatch("%s*([^,]+)%s*") do
		auto_blackitems[item] = true
	end
end
...
function XLootFrame:Update(no_snap, is_refresh)
	...
	if not self.built then
		...
		self.auto_blackitems = auto_blackitems
	...
	-- Autolooting options
	local auto, auto_items, auto_blackitems = auto, auto_items, auto_blackitems
	...
	for slot = 1, numloot do
		...
		elseif not icon then
			...
			if not is_refresh then
				-- Autolooting blacklist
				if auto.blacklist and auto_blackitems[name] then
					autoloot = false
				-- Autolooting everything
				elseif auto.all then
					autoloot = true
				-- Autolooting currency
				elseif auto.currency and (slotType == LOOT_SLOT_MONEY or slotType == LOOT_SLOT_CURRENCY) then
					autoloot = true
				-- Quest items
			...
			-- Show slot
			if not autoloot or is_refresh then
				our_slot = our_slot + 1
