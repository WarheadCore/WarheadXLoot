...
function addon:FillMonitorListItems()
	wipe(blacklist_items)
	wipe(whitelist_items)

	for item in opt.threshold_whitelist:gmatch("%s*([^,]+)%s*") do
		whitelist_items[item] = true
	end

	for item in opt.threshold_blacklist:gmatch("%s*([^,]+)%s*") do
		blacklist_items[item] = true
	end
end
...
function events.item(player, link, num)
	if link and link:match("|Hitem:") then -- Proper items
		...
		if not whitelist_items[name] then
			if (player == me and opt.threshold_own or opt.threshold_other) > quality then
				return
			end
		end

		if blacklist_items[name] then
			return
		end

		if not opt.show_ap and IsArtifactPowerItem(name) then
			return
		end
